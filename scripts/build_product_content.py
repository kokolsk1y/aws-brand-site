...
def main() -> None:
    ...
    def add(art, v, slug):
        ...
        # Материал-мусор STV (рекламное предложение вместо названия) → по семейству.
        # Делаем ДО правила покрытия, чтобы оно видело корректный материал.
        mat0 = specs.get("Материал", "")
        if mat0 and ("обеспечивает" in mat0.lower() or len(mat0) > 25):
            pref0 = re.match(r"[A-Z]+", base_key(art))
            specs["Материал"] = FRAME_MATERIAL.get(pref0.group() if pref0 else "", mat0)
        # ДИЗАЙН и АУРА: покрытие пластика строго по цвету (STV-проверено: белый —
        # глянцевое, всё остальное — матовое). Дотяжка от белого донора копировала
        # «глянцевое» и на чёрный/серый/золото — переопределяем. НО только для пластика:
        # стекло (G50/G51/PG51) глянцевое всегда, алюминий/дерево — матовые независимо
        # от цвета. УНО не трогаем — у него полные аутентичные данные (матовое+софт-тач).
        mat_low = specs.get("Материал", "").lower()
        is_hard = any(x in mat_low for x in ("стекло", "алюмин", "дерев"))
        if slug in ("design", "aura") and "Покрытие" in specs and not is_hard:
            specs["Покрытие"] = "Глянцевое" if article_color.get(art) == "white" else "Матовое"
        # Цвет — авторитетно из нашего каталога (STV-поле часто врёт: matte_black→
        # «чёрный», перепутанные white/чёрный/тёмно-серый, рекламные предложения).
        col_ru = COLOR_RU.get(article_color.get(art, ""))
        if col_ru and "Цвет" in specs:
            specs["Цвет"] = col_ru
        # Клавиши — по названию (STV-опечатка: A-002 одноклавишный с «Клавиш: 2»).
        mkl = re.search(r"(\d+)-кл", v["our_name"])
        if mkl and "Клавиш" in specs:
            specs["Клавиш"] = mkl.group(1)
        # Двойная розетка «Розетка 2-я» = 2 поста (STV сам себе противоречит 1/2).
        if re.match(r"Розетка 2-я", v["our_name"]) and "Постов" in specs:
            specs["Постов"] = "2"
        rows = [["Артикул", art]]
        ...
        # Пластиковые изделия ДИЗАЙН/АУРА (механизмы + рамки PD80/P50): покрытие у
        # белого/чёрного/серого разное, а описание общее на базу — убираем упоминание
        # поверхности, чтобы текст не противоречил таблице. Стекло (G50/G51/PG51) НЕ
        # трогаем — «глянцевая» верна для всех его цветов. + правка опечатки STV «однин».
        mat_low2 = specs.get("Материал", "").lower()
        is_plastic = ("поликарбонат" in mat_low2 or "пластик" in mat_low2) and "стекло" not in mat_low2
        if slug in ("design", "aura") and desc and is_plastic:
