...
//      - Рамки:       категория (ГОРИЗОНТАЛЬНЫЕ 1→5 постов → ДВОЙНЫЕ → ВЕРТИКАЛЬНЫЕ)
//                     → число постов → цвет.
//
...
// Категория рамки для сортировки: ГОРИЗОНТАЛЬНЫЕ (вкл. 1-постовые) → ДВОЙНЫЕ → ВЕРТИКАЛЬНЫЕ.
function frameCategoryRank(name) {
  if (/для двойной розетки/i.test(name)) return 1; // двойные — между гориз и верт
  if (/вертикальн/i.test(name)) return 2;          // вертикальные — в конец
  return 0;                                        // горизонтальные (вкл. 1-я)
}
...
function makeComparator(groupKey, colorOrder) {
  ...
  if (groupKey === 'frames') {
    return by(
      (x) => frameCategoryRank(x.name), // гориз → двойные → верт
      (x) => postsCount(x.name),        // внутри категории — по числу постов
      (x) => colorRank(x.color),
    );
  }
